package xiao.love.bar.component.segmentcontrol;
...
public class SegmentControl extends View {
    ...
    public SegmentControl(Context context, AttributeSet attrs, int defStyle){
        ...
        mDefaultTextColor = ta.getColorStateList(R.styleable.SegmentControl_defaultTextColor);
        mCheckTextColor = ta.getColorStateList(R.styleable.SegmentControl_checkTextColor);
        mStrokeColor = ta.getColorStateList(R.styleable.SegmentControl_strokeColor);
        mFillColor = ta.getColorStateList(R.styleable.SegmentControl_fillColor);
        ...
        if(mDefaultTextColor == null){
            mDefaultTextColor = new ColorStateList(new int[][]{{}}, new int[]{0xFF0099CC});
        }

        if(mCheckTextColor == null){
            mCheckTextColor = new ColorStateList(new int[][]{{}}, new int[]{0xFFFFFFFF});
        }

        if(mStrokeColor == null){
            mStrokeColor = new ColorStateList(new int[][]{{}}, new int[]{0xFF0099CC});
        }

        if(mFillColor == null){
            mFillColor = new ColorStateList(new int[][]{{}}, new int[]{0xFF0099CC});
        }

        //设置线条的颜色
        mBackgroundDrawable.setStrokeColor(mStrokeColor.getDefaultColor());
        ...
        //填充颜色
        mSelectedDrawable = new RadiusDrawable(mCornerRadius, false, mFillColor.getDefaultColor());

        //文字颜色
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaint.setTextSize(mTextSize);
        mPaint.setColor(mDefaultTextColor.getDefaultColor());
        ...
        if(mTexts != null && mTexts.length > 0){
            for(int i = 0; i < mTexts.length; i++){
                ...
                if(i == mCurrentIndex && mSelectedDrawable != null){
                    ...
                    mPaint.setColor(mCheckTextColor.getDefaultColor());
                }else{
                    mPaint.setColor(mDefaultTextColor.getDefaultColor());
                }
